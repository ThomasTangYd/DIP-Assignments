...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # 定义拉普拉斯核
    lap_kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=torch.float32, device=foreground_img.device).view(1, 1, 3, 3).repeat(3, 1, 1, 1)

    # 计算前景在背景上的偏移
    fg_ys, fg_xs = torch.where(foreground_mask[0, 0] > 0.5)
    bg_ys, bg_xs = torch.where(background_mask[0, 0] > 0.5)
    if fg_ys.numel() == 0 or bg_ys.numel() == 0:
        return loss

    dy, dx = bg_ys.min() - fg_ys.min(), bg_xs.min() - fg_xs.min()
    h_f, w_f = foreground_img.shape[-2:]
    h_b, w_b = blended_img.shape[-2:]

    # 重置区域
    y1, y2 = max(dy, 0), min(dy + h_f, h_b)
    x1, x2 = max(dx, 0), min(dx + w_f, w_b)
    if y1 >= y2 or x1 >= x2:
        return torch.tensor(0., device=foreground_img.device)
    
    src_y1, src_y2 = max(-dy, 0), max(-dy, 0) + (y2 - y1)
    src_x1, src_x2 = max(-dx, 0), max(-dx, 0) + (x2 - x1)

    # 创建对齐后的前景
    aligned_f = torch.zeros_like(blended_img)
    aligned_m = torch.zeros_like(background_mask)

    aligned_f[:, :, y1:y2, x1:x2] = foreground_img[:, :, src_y1:src_y2, src_x1:src_x2]
    aligned_m[:, :, y1:y2, x1:x2] = foreground_mask[:, :, src_y1:src_y2, src_x1:src_x2]

    # 计算拉普拉斯差异和L1损失
    lap_f = F.conv2d(aligned_f, lap_kernel, padding=1, groups=foreground_img.shape[1])
    lap_blend = F.conv2d(blended_img, lap_kernel, padding=1, groups=blended_img.shape[1])

    valid = (aligned_m * background_mask).expand(-1, foreground_img.shape[1], -1, -1).bool()

    loss = F.l1_loss(lap_blend[valid], lap_f[valid])
