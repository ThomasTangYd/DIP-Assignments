...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        # 纯 PyTorch 实现：计算成对欧氏距离（替代 pytorch3d 的 knn_points）
        dists = torch.cdist(points, points).squeeze(0)  # (N, N)
        # 排除自身（将自身距离设为无穷大）
        self_dists = torch.arange(self.n_points, device=dists.device)
        dists[self_dists, self_dists] = float('inf')
        # 取最近的 K 个邻居的距离
        dists, _ = torch.topk(dists, k=K, dim=1, largest=False)  # (N, K)
        
        # Use log space for unconstrained optimization
        # mean_dists = torch.mean(torch.sqrt(dists), dim=1, keepdim=True) * 2.
        # mean_dists = mean_dists.clamp(0.2*torch.median(mean_dists), 3.0*torch.median(mean_dists))  # Prevent infinite scales

        mean_dists = torch.mean(torch.sqrt(dists), dim=1, keepdim=True) * 2.
        # 过滤掉 inf 值后再 clamp
        valid_dists = mean_dists[mean_dists < float('inf')]
        if valid_dists.numel() > 0:
            median_dist = torch.median(valid_dists)
            mean_dists = mean_dists.clamp(0.2*median_dist, 3.0*median_dist)
        else:
            # 如果所有距离都是 inf，使用默认值
            mean_dists = mean_dists.fill_(0.1)
